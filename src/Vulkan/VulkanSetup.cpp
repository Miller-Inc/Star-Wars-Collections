...
namespace MillerInc::GPU
{
        ...
        VkQueue graphics_queue, VkCommandPool cmd_pool)
    {
        ...
        this->queue = graphics_queue;
        this->commandPool = cmd_pool;
    }
    ...
    VulkanSetup::TextureImage VulkanSetup::CreateTextureImage(const std::string& filename) const
    {
        ...
        viewInfo.image = textureImage; // your VkImage
        ...
        if (vkCreateImageView(device, &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
            M_LOGGER(Logger::LogGraphics, Logger::Error, "Failed to create image view!");
            return { VK_NULL_HANDLE, VK_NULL_HANDLE, VK_NULL_HANDLE, {0, 0} };
        }
    ...
    VkCommandBuffer VulkanSetup::BeginSingleTimeCommands() const {
    ...
    void VulkanSetup::EndSingleTimeCommands(VkCommandBuffer commandBuffer) const {
